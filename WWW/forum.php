...
$posts_per_page = 20;
$threads_per_page = 20;
$sections = array(1 => 'News', 2 => '<b>Support Abuse</b>', 3 => 'Tutorials', 4 => 'Screen Shots', 5 => 'Bugs In-Game', 6 => 'Player Reports', 7 => 'Donations', 8 => 'Requests', 9 => 'Ideas & Suggestions', 10 => 'Hyde Park');
$sections_desc = array(1 => 'Here you can comment all news.', 2 => 'Here you can report abusing GameMasters/Tutors.', 3 => 'Tutorials, example: How to create a guild.', 4 => 'Threads with pictures from game.', 5 => 'Here you can write about all bugs in-game or WebPage. Prizes waiting !', 6 => 'Here you can report all players who use the bot, or broke rules.', 7 => 'You lost your donation equipment by crash ? Post it here !', 8 => 'Do you have any questions about server ?<br> Maybe you don\'t know how to do Quest or use Command ? Post it here.', 9 => 'Your Ideas and Suggestion about server or WebPage.', 10 => 'About Everything and about Nothing.');
// END
...
if($action == '')
{
    ...
    {
        $last_post = $SQL->query("SELECT `players`.`name`, `z_forum`.`post_date`, `z_forum`.`post_topic`, `z_forum`.`id`, `z_forum`.`first_post` FROM `players`, `z_forum` WHERE `z_forum`.`section` = ".(int) $id." AND `players`.`id` = `z_forum`.`author_guid` ORDER BY `post_date` DESC LIMIT 1")->fetch();
	...
	$main_content .= '<small>&nbsp;Last Post at:<b> '.date('d.m.y H:i:s', $last_post['post_date']).'</b><br />&nbsp;&nbsp;&nbsp;in <a href="?subtopic=forum&action=show_thread&id='.$last_post['first_post'].'"> '.$last_post['post_topic'].'</a></small><br />&nbsp;by <a href="?subtopic=characters&name='.urlencode($last_post['name']).'">'.$last_post['name'].'</a>';      
  else
            $main_content .= '<b><center><font color="red">Board Empty</font></center></b>';
        $main_content .= '</td></tr>';
...
if($action == 'show_board')
{
    ...
    $main_content .= '<a href="?subtopic=forum">Boards</a> >> <b>'.$sections[$section_id].'</b><br /><br /><a href="?subtopic=forum&action=new_topic&section_id='.$section_id.'"><img src="images/topic.gif" border="0" /></a><br /><br />Page: '.$links_to_pages.'<br />';
 	$last_threads = $SQL->query("SELECT * FROM `players`, `z_forum` WHERE `players`.`id` = `z_forum`.`author_guid` AND `z_forum`.`section` = ".(int) $section_id." AND `z_forum`.`first_post` = `z_forum`.`id` ORDER BY `sticky` DESC, `post_date` DESC, `closed` DESC LIMIT ".$threads_per_page." OFFSET ".($page * $threads_per_page))->fetchAll();
    if(isset($last_threads[0]))
    {
        ...
        foreach($last_threads as $thread)
        {
            ...
            $stick = ""; $stix = '<span style="color:blue"><small>[STICK]</small></span>'; if ( $thread['sticky'] == 1 ) { $stick = '<strong><font color="green"><blink>STICKY: </blink></font></strong>'; $stix = '<span style="color:orange"><small>[UNSTICK]</small></span>'; }
			$cls = ""; $closed = '<span style="color:black"><small>[CLOSE]</small></span>'; if ( $thread['closed'] == 1 ) { $closed = '<span style="color:blackred"><small><blink>[OPEN]</blink></small></span>'; $cls = '<strong><font color="red"><b>[CLOSED]</b></font></strong>';}
            if($logged && $group_id_of_acc_logged >= $group_not_blocked)
                $main_content .= '<a href="?subtopic=forum&action=close_thread&id='.$thread['id'].'")">'.$closed.'</a> <a href="?subtopic=forum&action=stick_post&id='.$thread['id'].'" onclick="return confirm(\'Are you sure you want stick or unstick thread > '.$thread['post_topic'].' <?\')">'.$stix.'</a> <a href="?subtopic=forum&action=move_thread&id='.$thread['id'].'"\')"><span style="color:darkgreen"><small>[MOVE]</small></span></a> <a href="?subtopic=forum&action=remove_post&id='.$thread['id'].'" onclick="return confirm(\'Are you sure you want remove thread > '.$thread['post_topic'].' <?\')"><span style="color:red"><small>[REMOVE]</small></span></a><br/>';
                $main_content .= '<b>'.$stick.'<a STYLE="text-decoration:none" href="?subtopic=forum&action=show_thread&id='.$thread['id'].'">'.htmlspecialchars($thread['post_topic']).' </a>'.$cls.'</b><br /><small>'.htmlspecialchars(substr($thread['post_text'], 0, 50)).'...</small></td><td><a href="?subtopic=characters&name='.urlencode($thread['name']).'">'.$thread['name'].'</a></td><td>'.(int) $thread['replies'].'</td><td>'.(int) $thread['views'].'</td><td>';
            if($thread['last_post'] > 0)
            {
                ...
                if(isset($last_post['name']))
                    $main_content .= '<small>&nbsp;Last Post at:<b> '.date('d.m.y H:i:s', $last_post['post_date']).'</b></small><br />&nbsp;&nbsp;by <a href="?subtopic=characters&name='.urlencode($last_post['name']).'">'.$last_post['name'].'</a>';
                else
...
if($action  == 'show_thread')
{
    ...
    if(!empty($thread_name['name']))
    {
        ...
        $main_content .= '<br /><br /><strong><font size="10" color="red"><center><blink>Thread is Closed !</blink></center></font></strong><br /><br />Page: '.$links_to_pages.'<br /><table width="100%"><tr bgcolor="'.$config['site']['lightborder'].'" width="100%"><td colspan="2"><font size="4"><b>'.htmlspecialchars($thread_name['post_topic']).'</b></font><font size="1"><br />by <a href="?subtopic=characters&name='.urlencode($thread_name['name']).'">'.$thread_name['name'].'</a></font></td></tr><tr bgcolor="'.$config['site']['vdarkborder'].'"><td width="200"><font color="white" size="1"><b>Author</b></font></td><td>&nbsp;</td></tr>';
        ...
        $main_content .= '</table><br /><strong><font size="10" color="red"><center><blink>Thread is Closed !</blink></center></font></strong>';
    ...
    if($logged && $group_id_of_acc_logged >= $group_not_blocked)
    {
        ...
        if($post['id'] == $id)
        {
            if($post['id'] == $post['first_post'])
            {
                ...
                <tr bgcolor='.$config['site']['vdarkborder'].'><td class=white colspan=0><B><center>Move thread to the another board</center></B></td></tr>
                ...
                <br/><strong>Actual Board:</strong><font color="red"> <u>'.$sections[$post['section']].'</u></font><br/>
        ...
        if(canPost($account_logged) || $group_id_of_acc_logged >= $group_not_blocked)
        {
            ...
            if(isset($sections[$section_id]))
            {
                ...
                if(!$saved)
                {
					...
					if($section_id == 1)
						$main_content .= '<table border="0" width = "100%"><tr><td bgcolor="'.$config['site']['darkborder'].'"><b>Select icon:</b></td><td><table border="0" width = 100%" bgcolor="'.$config['site']['lightborder'].'"><tr><td><img src="'.$layout_name.'/images/news/icon_0.gif" width="20" align = "center"></td><td><img src="'.$layout_name.'/images/news/icon_1.gif" width="20"></td><td><img src="'.$layout_name.'/images/news/icon_2.gif" width="20"></td><td><img src="'.$layout_name.'/images/news/icon_3.gif" width="20"></td><td><img src="'.$layout_name.'/images/news/icon_4.gif" width="20"></td></tr><tr><td><input type="radio" name="icon_id" value="0" checked="checked"></td><td><input type="radio" name="icon_id" value="1"></td><td><input type="radio" name="icon_id" value="2"></td><td><input type="radio" name="icon_id" value="3"></td><td><input type="radio" name="icon_id" value="4"></td></tr></table></td></tr>';
					$main_content .= '<tr bgcolor="'.$config['site']['lightborder'].'"><td><b>Topic:</b></td><td><input type="text" name="topic" value="'.htmlspecialchars($post_topic).'" size="40" maxlength="60" /> (Optional)</td></tr>
                    <tr bgcolor="'.$config['site']['darkborder'].'"><td valign="top"><b>Message:</b><font size="1"><br />You can use:<br />[player]Nick[/player]<br />[url=http://address.com/]Address Search - Find Email and Addresses @ Address.com[/url]<br />[img]http://images.com/images3.gif[/img]<br />[code]Code[/code]<br />[b]<b>Text</b>[/b]<br />[i]<i>Text</i>[/i]<br />[u]<u>Text</u>[/u]<br />and smileys:<br />;) , :) , :D , :( , :rolleyes:<br />:cool: , :eek: , :o , :p</font></td><td><textarea rows="10" cols="40" name="text">'.htmlspecialchars($text).'</textarea><br />(Max. 15,000 letters)</td></tr>
                    <tr bgcolor="'.$config['site']['lightborder'].'"><td valign="top">Options:</td><td><label><input type="checkbox" name="smile" value="1"';
                    if($smile == 1)
                        $main_content .= ' checked="checked"';
