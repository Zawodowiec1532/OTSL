...
function onStepIn(cid, item, position, fromPosition)
	if getPlayerStorageValue(cid, lazaran) == 3 then
		...
		if getPlayerStorageValue(cid, item.uid) ~= 1 then
			...
			return (getPlayerStorageValue(cid, quest_storage) == 11) and doCreatureSay(cid, "".. towers[item.uid].txt .." Time to head back to Lazaran and show him what you got.", TALKTYPE_ORANGE_1) or doCreatureSay(cid, towers[item.uid].txt, TALKTYPE_ORANGE_1)
		end
	end
return true
end
