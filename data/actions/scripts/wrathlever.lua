...
function Close()
	for _, v in ipairs(FirstLamps) do
		doTransformItem(getTileItemById(v, 11435).uid, 11447)
...
function SecondClose()
	for _, v in ipairs(SecondLamps) do
		doTransformItem(getTileItemById(v, 11434).uid, 11449)
...
function onUse(cid, item, fromPosition, itemEx, toPosition)
	if item.actionid == 24232 and item.itemid == 10044 then
		for _, v in ipairs(FirstLamps) do
			doTransformItem(getTileItemById(v, 11447).uid, 11435)
			doTransformItem(item.uid, item.itemid + 1)
		end
		addEvent(Close, 10000)
	elseif item.actionid == 24233 and item.itemid == 10044 then
		for _, v in ipairs(SecondLamps) do
			doTransformItem(getTileItemById(v, 11449).uid, 11434)
			doTransformItem(item.uid, item.itemid + 1)
		end
