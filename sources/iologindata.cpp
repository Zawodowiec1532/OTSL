...
bool IOLoginData::playerMail(Creature* actor, std::string name, uint32_t townId, Item* item)
{
	...
	if(depot->getItemHoldingCount() > 2000 || item->getItemCount() > 2000)
	    return false;
...
//	{
//	    std::cout << "playerExists false " << name << ":D" <<  std::endl;
	    //return false;
//	}
	if(checkCache)
	{
...
bool IOLoginData::getGuidByName(uint32_t& guid, std::string& name, bool multiworld /*= false*/)
{
	...
	if(!isValidName(name, false))
	{
	    std::cout << "getGuidByName false" << std::endl;
//	    return false;
	}
	GuidCacheMap::iterator it = guidCacheMap.find(name);
	if(it != guidCacheMap.end())
...
bool IOLoginData::getGuidByNameEx(uint32_t& guid, bool &specialVip, std::string& name)
{
	...
	if(!isValidName(name, false))
	{
	    std::cout << "getGuidByNameEx false" << std::endl;
//	    return false;
	}
	Database* db = Database::getInstance();
	DBQuery query;
