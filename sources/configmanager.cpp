...
bool ConfigManager::load()
{
	...
	m_confNumber[FRAG_TIME] = getGlobalNumber("fragTime", 24);
	m_confBool[SEND_AFK] = getGlobalBool("sendAfk", true);

    m_confNumber[ROOK_LEVELTO] = getGlobalNumber("rookLevelTo", 5);
    m_confNumber[ROOK_TOWN] = getGlobalNumber("rookTownId", 1);
    m_confBool[ROOK_SYSTEM] = getGlobalBool("rookSystem", false);
    m_confBool[NO_DROP_BLESS] = getGlobalBool("noDropBless", false);
