...
bool Game::playerSetAttackedCreature(uint32_t playerId, uint32_t creatureId)
{
	...
	if(!player || player->isRemoved())
	 ...
	 if(player->hasCondition(CONDITION_EXHAUST, EXHAUST_FLOOD) && creatureId > 0)
 {
     return false;
 }
 player->addExhaust(100, EXHAUST_FLOOD);
...
//	if(time(NULL) - 10 < lastvip )
//	{
//	    std::cout << time(NULL) << " " << lastvip << std::endl;
//	    return false;
//	}
	if(!IOLoginData::getInstance()->getGuidByNameEx(guid, specialVip, name))
	{
		lastvip = time(NULL);
		player->vip++;
		std::cout << player->getName() << " probowal vipbug " << std::endl;
